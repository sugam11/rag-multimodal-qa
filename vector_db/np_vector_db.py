...
class NumpySearch(VectorDB):
    def __init__(self, embedder, data_set="MMQA"):
        ...
        if os.path.exists(STORE):
            ...
            if os.path.exists(os.path.join(STORE, f"meta_data_{data_set}.pkl")):
                with open(os.path.join(STORE, f"meta_data_{data_set}.pkl"), 'rb') as handle:
    ...
    def process_documents(self, data_set):
        ...
        if data_set == "WebQA":
            data = WebQAKnowledgeBase("/data/users/sgarg6/capstone/multimodalqa/MMQA_texts.jsonl",
                "/data/users/multimodalqa/MMQA_images.jsonl")
        else:
            data = MMQAKnowledgeBase("/data/users/sgarg6/capstone/multimodalqa/MMQA_texts.jsonl",
    ...
    def retrieve(self, text, result_type="hybrid", k=5):
        ...
        if result_type == "hybrid":
            ...
            top_k_docs = [self.meta_data[idx] for idx in top_k_idx[:2*k] if self.meta_data[idx]["type"] == "text"][:k]
        else:
            top_k_docs = [self.meta_data[idx] for idx in top_k_idx[:2*k] if self.meta_data[idx]["type"] == "img"][:k]
